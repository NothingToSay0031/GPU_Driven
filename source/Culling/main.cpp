...
int main(int argc, char** argv) {
  if (argc < 2) {
    printf("Usage: exe [path to glTF model]\n");
  ...
  // window
  WindowConfiguration wconf{1280, 800, "Meshlet GPU Culling",
  ...
  cstring shader_binaries_folder =
      temporary_name_buffer.append_use_f("%s/shaders /", RAPTOR_DATA_FOLDER);
  ...
  // Load frame graph and parse gpu techniques
  {
    cstring frame_graph_path =
        temporary_name_buffer.append_use_f("%s/%s", RAPTOR_WORKING_FOLDER, "graph.json");
    ...
    cstring dither_texture_path =
        temporary_name_buffer.append_use_f("%s/BayerDither4x4.png", RAPTOR_DATA_FOLDER);
    ...
    auto parse_technique = [&](cstring technique_name) {
      temporary_name_buffer.clear();
      cstring path =
          temporary_name_buffer.append_use_f("%s/%s", RAPTOR_SHADER_FOLDER, technique_name);
      render_resources_loader.load_gpu_technique(path, use_shader_cache);
    };

    cstring techniques[] = {"meshlet.json", "fullscreen.json", "main.json",   "pbr_lighting.json",
                            "dof.json",     "debug.json",      "culling.json"};
  ...
  while (!window.requested_exit) {
...

    {
      ...
      if (ImGui::Begin("Raptor ImGui")) {
        ...
        if (ImGui::Combo("Present Mode", &present_mode, raptor::PresentMode::s_value_names,
                         raptor::PresentMode::Count)) {
...

    {
      ...
      scene_data.previous_view_projection =
          scene_data.view_projection;  // Cache previous view projection
      ...
      scene_data.eye = vec4s{game_camera.camera.position.x, game_camera.camera.position.y,
      ...
      scene_data.frustum_planes[4] = normalize_plane(projection_transpose.col[3]);   // z + w  < 0;
      ...
      if (false) {
        ...
        getBoundsForAxis(vec3s{1, 0, 0}, {view_space_pos.x, view_space_pos.y, view_space_pos.z},
                         radius, game_camera.camera.near_plane, left, right);
        getBoundsForAxis(vec3s{0, 1, 0}, {view_space_pos.x, view_space_pos.y, view_space_pos.z},
        ...
        // left,right,bottom and top are in clip space (-1,1). Convert to 0..1 for UV, as used from
        // the optimized version to read the depth pyramid.
