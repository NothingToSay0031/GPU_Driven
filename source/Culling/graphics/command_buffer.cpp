...
void CommandBuffer::init(GpuDevice* gpu) {
  ...
  VkResult result = vkCreateDescriptorPool(
      device->vulkan_device, &pool_info, device->vulkan_allocation_callbacks, &vk_descriptor_pool);
...
void CommandBuffer::shutdown() {
  ...
  vkDestroyDescriptorPool(device->vulkan_device, vk_descriptor_pool,
                          device->vulkan_allocation_callbacks);
}
  ...
  // if ( !is_recording )
  {
    ...
    if (render_pass != current_render_pass) {
      if (device->dynamic_rendering_extension_present) {
        ...
        color_attachments_info.init(device->allocator, framebuffer->num_color_attachments,
        ...
        for (u32 a = 0; a < framebuffer->num_color_attachments; ++a) {
          ...
          color_attachment_info.imageLayout = device->synchronization2_extension_present
        ...
        if (has_depth_attachment) {
          ...
          depth_attachment_info.imageLayout =
              device->synchronization2_extension_present
                  ? VK_IMAGE_LAYOUT_ATTACHMENT_OPTIMAL_KHR
...
void CommandBuffer::draw_mesh_task_indirect(BufferHandle argument_buffer, u32 argument_offset,
  ...
  device->cmd_draw_mesh_tasks_indirect_count(vk_command_buffer, argument_buffer_->vk_buffer,
...
void CommandBuffer::barrier(const ExecutionBarrier& barrier) {
  ...
  if (barrier.new_barrier_experimental != u32_max) {
    ...
    VkAccessFlags source_access_flags = VK_ACCESS_NONE_KHR,
                  destination_access_flags = VK_ACCESS_NONE_KHR;
    ...
    for (u32 i = 0; i < barrier.num_image_barriers; ++i) {
...

      {
        ...
        pImageBarrier->subresourceRange.aspectMask =
            is_color ? VK_IMAGE_ASPECT_COLOR_BIT
    ...
    source_stage_mask = util_determine_pipeline_stage_flags(
        source_access_flags, barrier.source_pipeline_stage == PipelineStage::ComputeShader
                                 ? QueueType::Compute
                                 : QueueType::Graphics);
    destination_stage_mask = util_determine_pipeline_stage_flags(
        destination_access_flags, barrier.destination_pipeline_stage == PipelineStage::ComputeShader
                                      ? QueueType::Compute
                                      : QueueType::Graphics);

    vkCmdPipelineBarrier(vk_command_buffer, source_stage_mask, destination_stage_mask, 0, 0,
                         nullptr, barrier.num_memory_barriers, buffer_memory_barriers,
                         barrier.num_image_barriers, image_barriers);
    return;
  }
  ...
  VkAccessFlags destination_buffer_access_mask =
  ...
  for (u32 i = 0; i < barrier.num_image_barriers; ++i) {
    ...
    vk_barrier.subresourceRange.aspectMask =
        is_color ? VK_IMAGE_ASPECT_COLOR_BIT
                 : VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT;
  ...
  VkPipelineStageFlags source_stage_mask =
      to_vk_pipeline_stage((PipelineStage::Enum)barrier.source_pipeline_stage);
  VkPipelineStageFlags destination_stage_mask =
      to_vk_pipeline_stage((PipelineStage::Enum)barrier.destination_pipeline_stage);
  ...
  vkCmdPipelineBarrier(vk_command_buffer, source_stage_mask, destination_stage_mask, 0, 0, nullptr,
                       barrier.num_memory_barriers, buffer_memory_barriers,
                       barrier.num_image_barriers, image_barriers);
}
...
void CommandBuffer::push_marker(const char* name) {
  ...
  if (!device->debug_utils_extension_present) return;
...
void CommandBuffer::upload_texture_data(TextureHandle texture_handle, void* texture_data,
  ...
  util_add_image_barrier_ext(device, vk_command_buffer, texture, RESOURCE_STATE_COPY_SOURCE, 0, 1,
                             false, device->vulkan_transfer_queue_family,
                             device->vulkan_main_queue_family, QueueType::CopyTransfer,
                             QueueType::Graphics);
}
...
void CommandBuffer::copy_texture(TextureHandle src_, TextureHandle dst_, ResourceState dst_state) {
  ...
  for (int mip_index = 1; mip_index < dst->mip_level_count; ++mip_index) {
    util_add_image_barrier(device, vk_command_buffer, dst->vk_image, old_state,
                           RESOURCE_STATE_COPY_DEST, mip_index, 1, false);
    ...
    util_add_image_barrier(device, vk_command_buffer, dst->vk_image, RESOURCE_STATE_COPY_DEST,
...
void CommandBuffer::upload_buffer_data(BufferHandle buffer_handle, void* buffer_data,
  ...
  util_add_buffer_barrier_ext(
      device, vk_command_buffer, buffer->vk_buffer, RESOURCE_STATE_COPY_DEST,
      RESOURCE_STATE_UNDEFINED, copy_size, device->vulkan_transfer_queue_family,
      device->vulkan_main_queue_family, QueueType::CopyTransfer, QueueType::Graphics);
}
