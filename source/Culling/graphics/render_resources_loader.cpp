...
// Utility methods ////////////////////////////////////////////////////////
static u64 shader_concatenate(cstring filename, raptor::StringBuffer& path_buffer,
                              raptor::StringBuffer& shader_buffer,
                              raptor::Allocator* temp_allocator);
static VkBlendFactor get_blend_factor(const std::string factor);
static VkBlendOp get_blend_op(const std::string op);
static bool parse_gpu_pipeline(nlohmann::json& pipeline, raptor::PipelineCreation& pc,
                               raptor::StringBuffer& path_buffer,
                               raptor::StringBuffer& shader_buffer,
                               raptor::Allocator* temp_allocator, raptor::Renderer* renderer,
                               raptor::FrameGraph* frame_graph,
                               raptor::StringBuffer& pass_name_buffer,
                               const Array<VertexInputCreation>& vertex_input_creations,
                               FlatHashMap<u64, u16>& name_to_vertex_inputs, cstring technique_name,
                               bool use_cache, bool parent_technique);

// RenderResourcesLoader //////////////////////////////////////////////////
void RenderResourcesLoader::init(raptor::Renderer* renderer_,
                                 raptor::StackAllocator* temp_allocator_,
                                 raptor::FrameGraph* frame_graph_) {
  renderer = renderer_;
...
GpuTechnique* RenderResourcesLoader::load_gpu_technique(cstring json_path, bool use_shader_cache) {
  ...
  if (pipelines.is_array()) {
    ...
    for (u32 i = 0; i < size; ++i) {
      ...
      if (inherit_from.is_string()) {
        ...
        for (u32 ii = 0; ii < size; ++ii) {
          ...
          if (name == inherited_name) {
            add_pass =
                parse_gpu_pipeline(pipeline_i, pc, path_buffer, shader_code_buffer, temp_allocator,
                                   renderer, frame_graph, pass_name_buffer, vertex_input_creations,
                                   name_to_vertex_inputs, technique_creation.name, false, true);
            break;
          }
        }
      }
      ...
      add_pass =
          add_pass && parse_gpu_pipeline(pipeline, pc, path_buffer, shader_code_buffer,
                                         temp_allocator, renderer, frame_graph, pass_name_buffer,
                                         vertex_input_creations, name_to_vertex_inputs,
                                         technique_creation.name, use_shader_cache, false);
...
u64 shader_concatenate(cstring filename, raptor::StringBuffer& path_buffer,
                       raptor::StringBuffer& shader_buffer, raptor::Allocator* temp_allocator) {
  using namespace raptor;
...
bool parse_gpu_pipeline(nlohmann::json& pipeline, raptor::PipelineCreation& pc,
                        raptor::StringBuffer& path_buffer, raptor::StringBuffer& shader_buffer,
                        raptor::Allocator* temp_allocator, raptor::Renderer* renderer,
