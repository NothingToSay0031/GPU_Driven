...
void glTFScene::get_mesh_vertex_buffer(i32 accessor_index, u32 flag,
...
void glTFScene::fill_pbr_material(Renderer& renderer, glTF::Material& material,
  ...
  if (material.pbr_metallic_roughness != nullptr) {
    ...
    pbr_material.metallic_roughness_occlusion_factor.x =
        material.pbr_metallic_roughness->roughness_factor != glTF::INVALID_FLOAT_VALUE
            ? material.pbr_metallic_roughness->roughness_factor
            : 1.f;
    pbr_material.metallic_roughness_occlusion_factor.y =
        material.pbr_metallic_roughness->metallic_factor != glTF::INVALID_FLOAT_VALUE
            ? material.pbr_metallic_roughness->metallic_factor
            : 1.f;

    pbr_material.diffuse_texture_index =
        get_material_texture(gpu, material.pbr_metallic_roughness->base_color_texture);
    pbr_material.roughness_texture_index =
        get_material_texture(gpu, material.pbr_metallic_roughness->metallic_roughness_texture);
  }
  ...
  pbr_material.occlusion_texture_index = get_material_texture(
      gpu, (material.occlusion_texture != nullptr) ? material.occlusion_texture->index : -1);
  pbr_material.normal_texture_index = get_material_texture(
      gpu, (material.normal_texture != nullptr) ? material.normal_texture->index : -1);
...
void glTFScene::init(cstring filename, cstring path, Allocator* resident_allocator_,
                     StackAllocator* temp_allocator, AsynchronousLoader* async_loader) {
  ...
  for (u32 mi = 0; mi < gltf_scene.meshes_count; ++mi) {
    ...
    for (u32 p = 0; p < mesh.primitives_count; ++p) {
      ...
      f32 radius = raptor::max(glms_vec3_distance(position_max, bounding_center),
      ...
      get_mesh_vertex_buffer(position_accessor_index, 0, mesh.position_buffer, mesh.position_offset,
                             mesh.pbr_material.flags);
      get_mesh_vertex_buffer(tangent_accessor_index, DrawFlags_HasTangents, mesh.tangent_buffer,
                             mesh.tangent_offset, mesh.pbr_material.flags);
      get_mesh_vertex_buffer(normal_accessor_index, DrawFlags_HasNormals, mesh.normal_buffer,
                             mesh.normal_offset, mesh.pbr_material.flags);
      get_mesh_vertex_buffer(tex_coord_accessor_index, DrawFlags_HasTexCoords, mesh.texcoord_buffer,
                             mesh.texcoord_offset, mesh.pbr_material.flags);
      ...
      get_mesh_vertex_buffer(joints_accessor_index, DrawFlags_HasJoints, mesh.joints_buffer,
                             mesh.joints_offset, mesh.pbr_material.flags);
      get_mesh_vertex_buffer(weights_accessor_index, DrawFlags_HasWeights, mesh.weights_buffer,
                             mesh.weights_offset, mesh.pbr_material.flags);
      ...
      for (u32 m = 0; m < meshlet_count; ++m) {
        ...
        // Resize data array
        const u32 index_group_count = (local_meshlet.triangle_count * 3 + 3) / 4; // +3 for ceil
  ...
  for (u32 animation_index = 0; animation_index < gltf_scene.animations_count; ++animation_index) {
    ...
    for (u32 sampler_index = 0; sampler_index < gltf_animation.samplers_count; ++sampler_index) {
      ...
      sampler.interpolation_type =
          (raptor::AnimationSampler::Interpolation)gltf_sampler.interpolation;
...
void glTFScene::prepare_draws(Renderer* renderer, StackAllocator* scratch_allocator,
  ...
  if (renderer->gpu->mesh_shaders_extension_present) {
    ...
    DescriptorSetLayoutHandle layout = renderer->gpu->get_descriptor_set_layout(
        meshlet_technique->passes[0].pipeline, k_material_descriptor_set_index);
    ...
    for (u32 i = 0; i < k_max_frames; ++i) {
      ...
      ds_creation.buffer(scene_cb, 0)
          .buffer(meshes_sb, 2)
          .buffer(mesh_instances_sb, 10)
          .buffer(meshlets_sb, 1)
          .buffer(meshlets_data_sb, 3)
          .buffer(meshlets_vertex_pos_sb, 4)
          .buffer(meshlets_vertex_data_sb, 5)
      ...
      ds_creation.reset()
          ...
          .buffer(meshlets_vertex_pos_sb, 4)
          .buffer(meshlets_vertex_data_sb, 5)
          .buffer(mesh_task_indirect_late_commands_sb[i], 6)
