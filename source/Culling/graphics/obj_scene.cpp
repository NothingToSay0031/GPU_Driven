...
void ObjScene::init(cstring filename, cstring path, Allocator* resident_allocator_,
                    StackAllocator* temp_allocator, AsynchronousLoader* async_loader_) {
  ...
  for (u32 material_index = 0; material_index < assimp_scene->mNumMaterials; ++material_index) {
    ...
    if (aiGetMaterialString(material, AI_MATKEY_TEXTURE(aiTextureType_DIFFUSE, 0), &texture_file) ==
      ...
      hydra_material.diffuse_texture_index =
          load_texture(texture_file.C_Str(), path, temp_allocator);
    } else {
