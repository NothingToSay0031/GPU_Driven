...
static void copy_gpu_material_data(GpuMaterialData& gpu_mesh_data, const Mesh& mesh) {
  ...
  gpu_mesh_data.metallic_roughness_occlusion_factor =
      mesh.pbr_material.metallic_roughness_occlusion_factor;
...
void DepthPrePass::render(CommandBuffer* gpu_commands, RenderScene* render_scene) {
  ...
  if (render_scene->use_meshlets) {
    ...
    gpu_commands->bind_descriptor_set(
        &render_scene->mesh_shader_early_descriptor_set[buffer_frame_index], 1, nullptr, 0);

    gpu_commands->draw_mesh_task_indirect(
        render_scene->mesh_task_indirect_early_commands_sb[buffer_frame_index],
        offsetof(GpuMeshDrawCommand, indirectMS),
        render_scene->mesh_task_indirect_early_commands_sb[buffer_frame_index], 0,
...
void GBufferPass::render(CommandBuffer* gpu_commands, RenderScene* render_scene) {
  ...
  if (render_scene->use_meshlets) {
    ...
    gpu_commands->draw_mesh_task_indirect(
        render_scene->mesh_task_indirect_early_commands_sb[buffer_frame_index],
        offsetof(GpuMeshDrawCommand, indirectMS),
        render_scene->mesh_task_indirect_count_early_sb[buffer_frame_index], 0,
        render_scene->mesh_instances.size, sizeof(GpuMeshDrawCommand));
  } else {
    Material* last_material = nullptr;
    for (u32 mesh_index = 0; mesh_index < mesh_instance_draws.size; ++mesh_index) {
...
void LateGBufferPass::prepare_draws(RenderScene& scene, FrameGraph* frame_graph,
  ...
  material_creation.set_name("material_no_cull_late")
...
void LateGBufferPass::render(CommandBuffer* gpu_commands, RenderScene* render_scene) {
  ...
  if (render_scene->use_meshlets) {
    ...
    gpu_commands->bind_descriptor_set(
        &render_scene->mesh_shader_late_descriptor_set[buffer_frame_index], 1, nullptr, 0);

    gpu_commands->draw_mesh_task_indirect(
        render_scene->mesh_task_indirect_late_commands_sb[buffer_frame_index],
        offsetof(GpuMeshDrawCommand, indirectMS),
        render_scene->mesh_task_indirect_count_late_sb[buffer_frame_index], 0,
        render_scene->mesh_instances.size, sizeof(GpuMeshDrawCommand));
  }
}
...
void LightPass::prepare_draws(RenderScene& scene, FrameGraph* frame_graph,
  ...
  ds_creation.buffer(scene.scene_cb, 0)
      .buffer(mesh.pbr_material.material_buffer, 1)
      .set_layout(layout);
...
void TransparentPass::render(CommandBuffer* gpu_commands, RenderScene* render_scene) {
  ...
  if (render_scene->use_meshlets) {
    ...
    gpu_commands->draw_mesh_task_indirect(
        ...
        render_scene->mesh_task_indirect_count_early_sb[buffer_frame_index], indirect_count_offset,
        render_scene->mesh_instances.size, sizeof(GpuMeshDrawCommand));
  } else {
    Material* last_material = nullptr;
    for (u32 mesh_index = 0; mesh_index < mesh_instance_draws.size; ++mesh_index) {
...
void DebugPass::prepare_draws(RenderScene& scene, FrameGraph* frame_graph,
  ...
  // Prepare gpu debug line resources
  {
    ...
    set_creation.set_layout(layout)
        ...
        .buffer(scene.debug_line_sb, 20)
        .buffer(scene.debug_line_count_sb, 21)
        .buffer(scene.debug_line_commands_sb, 22);
    ...
    layout = renderer->gpu->get_descriptor_set_layout(main_technique->passes[pass_index].pipeline,
...
void MeshPass::prepare_draws(RenderScene& scene, FrameGraph* frame_graph,
                             Allocator* resident_allocator, StackAllocator* scratch_allocator) {
...
void CullingEarlyPass::render(CommandBuffer* gpu_commands, RenderScene* render_scene) {
  ...
  MapBufferParameters cb_map{render_scene->mesh_task_indirect_count_early_sb[buffer_frame_index], 0,
                             0};
  ...
  const Buffer* count_sb = renderer->gpu->access_buffer(
      render_scene->mesh_task_indirect_count_early_sb[buffer_frame_index]);
  util_add_buffer_barrier(renderer->gpu, gpu_commands->vk_command_buffer, count_sb->vk_buffer,
                          RESOURCE_STATE_INDIRECT_ARGUMENT, RESOURCE_STATE_UNORDERED_ACCESS,
                          count_sb->size);

  gpu_commands->bind_descriptor_set(&frustum_cull_descriptor_set[buffer_frame_index], 1, nullptr,
                                    0);
      ...
      renderer->resource_cache.techniques.get(hash_calculate("culling"));
  {
    ...
    for (u32 i = 0; i < k_max_frames; ++i) {
      ...
      ds_creation.buffer(scene.meshes_sb, 2)
          ...
          .buffer(scene.scene_cb, 0)
          .buffer(scene.mesh_task_indirect_count_early_sb[i], 11)
...
void CullingLatePass::render(CommandBuffer* gpu_commands, RenderScene* render_scene) {
  ...
  u32 buffer_frame_index = renderer->gpu->current_frame;
  MapBufferParameters cb_map{render_scene->mesh_task_indirect_count_late_sb[buffer_frame_index], 0,
                             0};
  GpuMeshDrawCounts* count_data = (GpuMeshDrawCounts*)renderer->gpu->map_buffer(cb_map);
  if (count_data) {
    *count_data = mesh_draw_counts;
  ...
  const Buffer* count_sb = renderer->gpu->access_buffer(
      render_scene->mesh_task_indirect_count_late_sb[buffer_frame_index]);
  util_add_buffer_barrier(renderer->gpu, gpu_commands->vk_command_buffer, count_sb->vk_buffer,
                          RESOURCE_STATE_INDIRECT_ARGUMENT, RESOURCE_STATE_UNORDERED_ACCESS,
                          count_sb->size);
...
void RenderScene::upload_gpu_data() {
  ...
  GpuMeshInstanceData* gpu_mesh_instance_data =
      (GpuMeshInstanceData*)renderer->gpu->map_buffer(cb_map);
...
void FrameRenderer::init(Allocator* resident_allocator_, Renderer* renderer_,
  ...
  frame_graph->builder->register_render_pass("mesh_occlusion_early_pass",
                                             &mesh_occlusion_early_pass);
...
void FrameRenderer::prepare_draws(StackAllocator* scratch_allocator) {
  ...
  gbuffer_pass_early.prepare_draws(*scene, frame_graph, renderer->gpu->allocator,
                                   scratch_allocator);
  ...
  mesh_occlusion_early_pass.prepare_draws(*scene, frame_graph, renderer->gpu->allocator,
                                          scratch_allocator);
  mesh_occlusion_late_pass.prepare_draws(*scene, frame_graph, renderer->gpu->allocator,
                                         scratch_allocator);
  depth_pyramid_pass.prepare_draws(*scene, frame_graph, renderer->gpu->allocator,
                                   scratch_allocator);
  ...
  DescriptorSetLayoutHandle descriptor_set_layout = renderer->gpu->get_descriptor_set_layout(
      fullscreen_tech->passes[0].pipeline, k_material_descriptor_set_index);
