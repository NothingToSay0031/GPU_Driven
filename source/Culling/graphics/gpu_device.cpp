...
void GpuDevice::init(const GpuDeviceCreation& creation) {
  ...
  if (synchronization2_extension_present) {
    ...
    cmd_pipeline_barrier2 = (PFN_vkCmdPipelineBarrier2KHR)vkGetDeviceProcAddr(
  ...
  if (mesh_shaders_extension_present) {
    ...
    cmd_draw_mesh_tasks_indirect = (PFN_vkCmdDrawMeshTasksIndirectNV)vkGetDeviceProcAddr(
        vulkan_device, "vkCmdDrawMeshTasksIndirectNV");
    cmd_draw_mesh_tasks_indirect_count = (PFN_vkCmdDrawMeshTasksIndirectCountNV)vkGetDeviceProcAddr(
  ...
  descriptor_set_layouts.init(allocator, k_descriptor_set_layouts_pool_size,
...
static void vulkan_create_texture(GpuDevice& gpu, const TextureCreation& creation,
  ...
  tvc.set_mips(0, creation.mip_level_count)
      .set_array(0, creation.array_layer_count)
...
PipelineHandle GpuDevice::create_pipeline(const PipelineCreation& creation,
  ...
  for (u32 l = 0; l < num_active_layouts; ++l) {
    ...
    pipeline->descriptor_set_layout[l] =
...
void GpuDevice::fill_write_descriptor_sets(
    GpuDevice& gpu, const DescriptorSetLayout* descriptor_set_layout,
    VkDescriptorSet vk_descriptor_set, VkWriteDescriptorSet* descriptor_write,
...
DescriptorSetHandle GpuDevice::create_descriptor_set(const DescriptorSetCreation& creation) {
  ...
  fill_write_descriptor_sets(*this, descriptor_set_layout, descriptor_set->vk_descriptor_set,
                             descriptor_write, buffer_info, image_info,
                             vk_default_sampler->vk_sampler, num_resources, creation.resources,
                             creation.samplers, creation.bindings);
...
void GpuDevice::destroy_buffer(BufferHandle buffer) {
  if (buffer.index < buffers.pool_size) {
    resource_deletion_queue.push(
        {ResourceUpdateType::Buffer, buffer.index, current_frame + k_max_frames, 1});
  } else {
...
void GpuDevice::create_swapchain() {
  ...
  for (u32 iv = 0; iv < vulkan_swapchain_image_count; iv++) {
    ...
    TextureCreation depth_texture_creation;  // = { nullptr, swapchain_width, swapchain_height, 1,
                                             // 1, 0, VK_FORMAT_D32_SFLOAT, TextureType::Texture2D,
                                             // k_invalid_texture, "DepthImage_Texture" };
...
void GpuDevice::update_descriptor_set_instant(const DescriptorSetUpdate& update) {
  ...
  fill_write_descriptor_sets(
      *this, descriptor_set_layout, descriptor_set->vk_descriptor_set, descriptor_write,
      buffer_info, image_info, vk_default_sampler->vk_sampler, num_resources,
      descriptor_set->resources, descriptor_set->samplers, descriptor_set->bindings);
...
void GpuDevice::resize_texture(TextureHandle texture, u32 width, u32 height) {
  ...
  tc.set_flags(vk_texture->flags)
      ...
      .set_size(width, height, vk_texture->depth)
  ...
  if (timeline_semaphore_extension_present) {
    ...
    if (synchronization2_extension_present) {
      ...
      VkSemaphoreSubmitInfoKHR wait_semaphores[]{
          {VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr, vulkan_image_acquired_semaphore, 0,
          ...
          {VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr, vulkan_compute_semaphore,
           last_compute_semaphore_value, VK_PIPELINE_STAGE_2_VERTEX_ATTRIBUTE_INPUT_BIT_KHR, 0},
          {VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO_KHR, nullptr, vulkan_graphics_semaphore,
           absolute_frame - (k_max_frames - 1), VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT_KHR, 0},
      };
