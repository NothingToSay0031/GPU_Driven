...
static void test_sphere_aabb(raptor::GameCamera& game_camera) {
  ...
  raptor::get_bounds_for_axis(vec3s{1, 0, 0},
                              {view_space_pos.x, view_space_pos.y, view_space_pos.z}, radius,
                              game_camera.camera.near_plane, left, right);
  raptor::get_bounds_for_axis(vec3s{0, 1, 0},
                              {view_space_pos.x, view_space_pos.y, view_space_pos.z}, radius,
...
int main(int argc, char** argv) {
  ...
  // window
  WindowConfiguration wconf{1280, 800, "Light Cluster",
  ...
  while (!window.requested_exit) {
...

    {
      ...
      scene_data.light0_data0 =
          vec4s{light_position.x, light_position.y, light_position.z, light_radius};
      ...
      if (scene->lights_tiles_sb[0].index != k_invalid_buffer.index) {
        lights_tiles_buffer =
            renderer.gpu->access_buffer(scene->lights_tiles_sb[gpu.current_frame]);
      }
