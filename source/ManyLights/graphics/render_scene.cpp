...
void DepthPyramidPass::prepare_draws(RenderScene& scene, FrameGraph* frame_graph,
  ...
  sc.set_address_mode_uvw(VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE,
      ...
      .set_reduction_mode(VK_SAMPLER_REDUCTION_MODE_MAX) // Use max for depth pyramid
...
void GBufferPass::pre_render(u32 current_frame_index, CommandBuffer* gpu_commands,
  ...
  if (render_scene->use_meshlets_emulation) {
    ...
    gpu_commands->buffer_barrier(meshlet_index_buffer, RESOURCE_STATE_INDEX_BUFFER,
    ...
    gpu_commands->buffer_barrier(meshlet_index_buffer, RESOURCE_STATE_UNORDERED_ACCESS,
...
void GBufferPass::prepare_draws(RenderScene& scene, FrameGraph* frame_graph,
  ...
  for (u32 i = 0; i < k_max_frames; ++i) {
    ...
    ds_creation.set_layout(layout_generate_ib)
        .buffer(scene.meshlets_sb, 1)
        .buffer(scene.meshlets_data_sb, 3)
  ...
  if (last_lights_buffer.index != scene.lights_tiles_sb[0].index) {
    ...
    for (u32 i = 0; i < k_max_frames; ++i) {
      ...
      ds_creation.reset()
          ...
          .buffer(scene.lights_lut_sb[i], 20)
          .buffer(scene.lights_list_sb, 21)
          .buffer(scene.lights_tiles_sb[i], 22)
          .buffer(scene.lights_indices_sb[i], 25)
          .set_layout(layout);
      ...
      if (scene.use_meshlets) {
        scene.renderer->gpu->destroy_descriptor_set(
            scene.mesh_shader_transparent_descriptor_set[i]);
        ...
        DescriptorSetLayoutHandle transparent_layout = renderer->gpu->get_descriptor_set_layout(
            transparent_technique->passes[meshlet_technique_index].pipeline,
        ...
        ds_creation.reset()
            ...
            .buffer(scene.meshes_sb, 2)
            .buffer(scene.meshlets_data_sb, 3)
            .buffer(scene.meshlets_vertex_pos_sb, 4)
            .buffer(scene.meshlets_vertex_data_sb, 5)
            .buffer(scene.mesh_task_indirect_early_commands_sb[i], 6)
            .buffer(scene.mesh_task_indirect_count_early_sb[i], 7)
            .buffer(scene.mesh_instances_sb, 10)
            .buffer(scene.mesh_bounds_sb, 12)
...
//
// MeshPass ////////////////////////////////////////////////////////
void MeshPass::render(u32 current_frame_index, CommandBuffer* gpu_commands,
                      RenderScene* render_scene) {
  if (!enabled) return;
...
void RenderScene::upload_gpu_data(UploadGpuDataContext& context) {
  ...
  for (u32 i = 0; i < k_num_lights; ++i) {
    ...
    // rprint("Light Z %f, Zmin %f, Zmax %f\n", sorted_light.projected_z, sorted_light.projected_z_min,
    //        sorted_light.projected_z_max);
  }

  qsort(sorted_lights.data, k_num_lights, sizeof(SortedLight), sorting_light_fn);
  // for (u32 i = 0; i < k_num_lights; ++i) {
  //   rprint("Light %u Z %f\n", sorted_lights[i].light_index, sorted_lights[i].projected_z);
  // }
  ...
  for (u32 i = 0; i < k_num_lights; ++i) {
    ...
    // TODO: check if light is visible
