...
void glTFScene::prepare_draws(Renderer* renderer, StackAllocator* scratch_allocator,
  ...
  // if ( renderer->gpu->mesh_shaders_extension_present )
  {
    ...
    DescriptorSetLayoutHandle layout = meshlet_index != u16_max
                                           ? renderer->gpu->get_descriptor_set_layout(
                                                 meshlet_technique->passes[meshlet_index].pipeline,
    ...
    DescriptorSetLayoutHandle meshlet_emulation_layout = renderer->gpu->get_descriptor_set_layout(
        meshlet_technique->passes[meshlet_emulation_index].pipeline,
        k_material_descriptor_set_index);
    ...
    for (u32 i = 0; i < k_max_frames; ++i) {
      ...
      ds_creation.reset()
          ...
          .buffer(mesh_task_indirect_early_commands_sb[i], 6)
          .buffer(mesh_task_indirect_count_early_sb[i], 7)
          .buffer(meshlets_instances_sb[i], 9)
          .buffer(mesh_bounds_sb, 12)
          .buffer(debug_line_sb, 20)
          .buffer(debug_line_count_sb, 21)
          .buffer(debug_line_commands_sb, 22)
  ...
  for (u32 i = 0; i < k_num_lights; ++i) {
    ...
    if (x <= 0) {
      new_light.radius = 3.f;
      new_light.intensity = 5.0f;
    }
