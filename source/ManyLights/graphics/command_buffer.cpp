...
void CommandBuffer::buffer_barrier(BufferHandle buffer_handle, ResourceState old_state,
                                   ResourceState new_state, QueueType::Enum source_queue_type,
                                   QueueType::Enum destination_queue_type) {
  Buffer* buffer = device->access_buffer(buffer_handle);
  util_add_buffer_barrier_ext(device, vk_command_buffer, buffer->vk_buffer,
...
void CommandBuffer::barrier(const ExecutionBarrier& barrier) {
  ...
  if (device->synchronization2_extension_present) {
    ...
    for (u32 i = 0; i < barrier.num_image_barriers; ++i) {
      ...
      barrier.subresourceRange.aspectMask = TextureFormat::has_depth_or_stencil(texture->vk_format)
