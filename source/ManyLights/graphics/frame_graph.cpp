  ...
  for (u32 n = 0; n < nodes.size; ++n) {
    ...
    if (node->compute) {
      ...
      for (u32 i = 0; i < node->inputs.size; ++i) {
        ...
        if (input_resource->type == FrameGraphResourceType_Texture) {
          Texture* texture =
              gpu_commands->device->access_texture(resource->resource_info.texture.handle);

          util_add_image_barrier(gpu_commands->device, gpu_commands->vk_command_buffer, texture,
      ...
      for (u32 i = 0; i < node->inputs.size; ++i) {
        ...
        if (input_resource->type == FrameGraphResourceType_Texture) {
          ...
          if (!TextureFormat::has_depth_or_stencil(texture->vk_format)) {
            ...
            util_add_image_barrier(gpu_commands->device, gpu_commands->vk_command_buffer, texture,
                                   RESOURCE_STATE_DEPTH_WRITE, 0, 1, true);
          }
        }
      }
