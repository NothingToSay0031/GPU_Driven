...
// TODO:
static void generate_mipmaps(raptor::Texture* texture, raptor::CommandBuffer* cb,
                             bool from_transfer_queue) {
  using namespace raptor;

  if (texture->mip_level_count > 1) {
    util_add_image_barrier(
        cb->device, cb->vk_command_buffer, texture->vk_image,
  ...
  for (int mip_index = 1; mip_index < texture->mip_level_count; ++mip_index) {
    util_add_image_barrier(cb->device, cb->vk_command_buffer, texture->vk_image,
...
void Renderer::add_texture_update_commands(u32 thread_id) {
  ...
  for (u32 i = 0; i < num_textures_to_update; ++i) {
    ...
    util_add_image_barrier_ext(cb->device, cb->vk_command_buffer, texture->vk_image,
                               RESOURCE_STATE_COPY_DEST, RESOURCE_STATE_COPY_SOURCE, 0, 1, false,
                               gpu->vulkan_transfer_queue_family, gpu->vulkan_main_queue_family,
