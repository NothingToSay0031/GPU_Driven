...
void BlobSerializer::write_common(Allocator* allocator_,
                                  u32 serializer_version_, sizet size) {
  ...
  if (is_reading) {
    ...
    if (source_data_offset > 0) {
      ...
      *data = data_memory + allocated_offset;
      ...
      char* source_data =
          blob_memory + cached_serialized + source_data_offset - 4;
...
char* BlobSerializer::allocate_static(sizet size) {
  if (allocated_offset + size > total_size) {
    rprint(
        "Blob allocation error: allocated, requested, total - %u + %u > %u\n",
...
void BlobSerializer::allocate_and_set(RelativeString& string, cstring format,
...
#if (_MSC_VER)
  int written_chars =
      vsnprintf_s(&destination_memory[allocated_offset],
                  total_size - allocated_offset, _TRUNCATE, format, args);
#else
  int written_chars = vsnprintf(&destination_memory[allocated_offset],
                                total_size - allocated_offset, format, args);
#endif
  ...
  if (written_chars < 0) {
    rprint(
        "New string too big for current buffer! Please allocate more size.\n");
  }
...
void BlobSerializer::allocate_and_set(RelativeString& string, char* text,
                                      u32 length) {
